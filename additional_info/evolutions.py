...
def one_ev_line(div: Tag):
    ...
    levels_list: List[Tag] = div.find_all('span', attrs={'class':'infocard-arrow'})
    poke_ev_dict = {}

    for combo in zip_longest(info_list, levels_list):
        poke_ev_dict[combo[0].text.split(' ')[1]] = combo[1].text if combo[1] else ''
...
def get_evolutions(soup: BeautifulSoup) -> dict:
    ...
    ev_divs: List[Tag] = soup.find_all('div', attrs={'class': 'infocard-list-evo'})
    if ev_divs:
        if len(ev_divs) == 1:
